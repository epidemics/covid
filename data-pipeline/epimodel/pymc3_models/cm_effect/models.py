...
class BaseCMModel(Model):
    ...
    def run(self, draws: int, chains=2, cores=2, **kwargs):
        ...
        with self.model:
            self.trace = pm.sample(draws=draws, chains=chains, cores=cores, init="adapt_diag", **kwargs)
...
class CMCombinedModel(BaseCMModel):
    ...
    def __init__(
        ...
        for r in range(self.num_regions):
            for d in range(self.num_days):
                ...
                if self.data.NewCases.mask[r, d] == False and d > self.CMDelayCut and not np.isnan(
                        self.data.Confirmed.data[r, d]) and d < (self.num_days - 7):
        ...
        for r in range(self.num_regions):
            for d in range(self.num_days):
                ...
                if self.data.NewDeaths.mask[r, d] == False and d > self.CMDelayCut and not np.isnan(
                        self.data.Deaths.data[r, d]):
    ...
    def build_model(
        ...
        with self.model:
            if cm_prior == 'normal':
                self.CM_Alpha = pm.Normal("CM_Alpha", 0, cm_prior_sigma, shape=(self.num_countermeasures,))

            if cm_prior == 'half_normal':
                self.CM_Alpha = pm.HalfNormal("CM_Alpha", cm_prior_sigma, shape=(self.num_countermeasures,))

            self.CMReduction = pm.Deterministic("CMReduction", tensor.exp((-1.0) * self.CM_Alpha))
            ...
            self.create_deterministic(
                "GrowthReduction", tensor.sum(self.ActiveCMReduction, axis=1), plot_trace=False
            )

            self.ExpectedLogR = self.create_deterministic(
                "ExpectedLogR",
                tensor.reshape(self.RegionLogR, (self.num_observed_regions, 1)) - self.GrowthReduction,
                plot_trace=False,
            )
            ...
            self.ExpectedGrowth = self.create_deterministic(
                "ExpectedGrowth",
                si_beta * (pm.math.exp(self.ExpectedLogR / si_alpha) - tensor.ones_like(self.ExpectedLogR)),
                plot_trace=False
            )
            ...
            self.create_deterministic("Z1C", self.GrowthCases - self.ExpectedGrowth, plot_trace=False)
            self.create_deterministic("Z1D", self.GrowthDeaths - self.ExpectedGrowth, plot_trace=False)

            self.InitialSizeCases_log = pm.Normal("InitialSizeCases_log", 0, 50, shape=(self.num_observed_regions,))
            self.InfectedCases_log = pm.Deterministic("InfectedCases_log", tensor.reshape(self.InitialSizeCases_log, (
                self.num_observed_regions, 1)) + self.GrowthCases.cumsum(axis=1))
            ...
            self.ObservedCases = pm.NegativeBinomial(
                "ObservedCases",
                mu=self.ExpectedCases.reshape((self.num_observed_regions * self.num_days,))[self.all_observed_active],
                alpha=self.Phi,
                shape=(len(self.all_observed_active),),
                observed=self.data.NewCases.data.reshape((self.num_observed_regions * self.num_days,))[self.all_observed_active]
            )

            self.Z2C = pm.Deterministic(
                "Z2C",
                self.ObservedCases - self.ExpectedCases.reshape((self.num_observed_regions * self.num_days,))[self.all_observed_active]
            )

            self.InitialSizeDeaths_log = pm.Normal("InitialSizeDeaths_log", 0, 50, shape=(self.num_observed_regions,))
            self.InfectedDeaths_log = pm.Deterministic(
                "InfectedDeaths_log",
                tensor.reshape(self.InitialSizeDeaths_log, (self.num_observed_regions, 1)) + self.GrowthDeaths.cumsum(axis=1)
            )
            ...
            self.ObservedDeaths = pm.NegativeBinomial(
                "ObservedDeaths",
                mu=self.ExpectedDeaths.reshape((self.num_observed_regions * self.num_days,))[self.all_observed_deaths],
                alpha=self.Phi,
                shape=(len(self.all_observed_deaths),),
                observed=self.data.NewDeaths.data.reshape((self.num_observed_regions * self.num_days,))[self.all_observed_deaths]
            )

            self.create_deterministic(
                "Z2D",
                self.ObservedDeaths - self.ExpectedDeaths.reshape((self.num_observed_regions * self.num_days,))[
    ...
    def region_forward_pass(self, region):
        ...
        predicted_cases_result = self.produce_confidence_intervals(
            pm.NegativeBinomial
                .dist(mu=ec, alpha=np.repeat(np.array([self.trace.Phi_1]), ec.shape[1], axis=0).T)
                .random()
        ).add_prefix("PredictedNewCases_")

        ed = self.trace.ExpectedDeaths[:, country_idx, :]
        _, num_days = ed.shape
        dist = pm.NegativeBinomial.dist(mu=ed + 1e-3, alpha=np.repeat(np.array([self.trace.Phi_1]), num_days, axis=0).T)
        ...
        try:
            ...
            log.warning(f"Region forward pass for region {region} failed for expected death", ex)
            ed_output = np.ones_like(ids) * 10 ** -5
            ids = np.ones_like(ids) * 10 ** -5

        infected_deaths_result = self.produce_confidence_intervals(
            ids
        ).add_prefix("DailyInfectedDeaths_")
        ...
        full_result = predictions \
            .merge(recorded, how='outer', left_index=True, right_index=True)
        return full_result
