...
function addHistoricalData(plotlyGraph, pop){
  let cfr = 0.016;
  let cfr_log_cv = 0.5; // coefficent of variance (relative sd) of cfr
  ...
  d3.json(
    ...
    // error bars
    let errors = {
      y: deaths.map((v) => applyVariance(v/cfr/pop, [cfr_log_cv, 3/Math.sqrt(v)], 1)),
      x: dates,
      mode: "lines",
    ...
    for(let i = deaths.length-1; i >= 0; i--){
      let v = deaths[i];
      errors.y.push(applyVariance(v/cfr/pop, [1/2, 10/Math.sqrt(v)],-1));
